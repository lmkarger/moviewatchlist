...
export const MovieCard = ({items: initialItems ,className, Rated}: {
  ...
  async function handleDelete(id: number) {
    ...
    if (Rated){
      const { error } = await supabase.from("moviesWatched").delete().eq("id", id);
      ...
      if (error) {
        console.error(error);
            ...
            const { error } = await supabase.from("movieWatchlist").delete().eq("id", id);
...
export const MovieTitle = ({
  ...
  return (
    <h4 className={cn("text-zinc-100 font-bold tracking-wide mt-4 mb-4 text-center", className)}>
      {children}
    </h4>
  );
};
export const MovieRating = ({
  ...
  return (
    <p className={cn("mt-8 text-zinc-400 tracking-wide leading-relaxed text-sm text-center", className)}>
      {children}/10
    </p>
  );
};
